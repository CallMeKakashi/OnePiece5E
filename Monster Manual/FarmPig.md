---
type: monster
status: draft
publish: true
aliases:
  - Dire Thornboar
foundry_actor_id: "e5oRsFiw5cfVq8Nv"
foundry_live_slug: "farmpig"
---
# FarmPig

## Visuals

![[Attachments/monsters/farmpig.png|registry-image]]

**Look:** Lunafang **mutated pig** grown into a spined boar — shaggy dark-brown fur, small angry eyes, curling **ivory tusks**, and a ridge of **yellow bone horns** from snout along the spine (**Dire Thornboar** silhouette).

*Exported once from Foundry actor `e5oRsFiw5cfVq8Nv` (`Windows-sync/Mafia Arc/token_4 (2).png`). Stats sync separately; image is vault-local.*

## Role

**Lunafang-mutated farm pig** from the **West Town** fertilizer incident (same feed poisoning as **[[FarmBull]]** and **[[Hydra Goose]]**). Grew into a spined, relentless boar-like predator (**Spine Burst**, **Relentless** — drops to 1 HP once per day).

Part of Nikolai's **Lunafang** pipeline on **Juniper**; Simon and Zara's cover-up killings apply to the whole farm scene.

## Appearances

- **West Town farm** — Same encounter as **[[Hydra Goose]]**; killed with the other mutates by [[Matthew -The Jack- Burgess|Jack]], [[Tray]], and [[Midori]] (transcript pending).

## Related

- [[Hydra Goose]] · [[FarmBull]]
- [[Dr Nikolai Tesla]] · [[LunaFang Serum]]
- [[Simon The One Armed Tyrant]] · [[Zara Tideborn]]
- [[Matthew -The Jack- Burgess]] · [[Tray]] · [[Midori]]
- [[Sessions/Session 27]]
- [[Monster Manual|Monster Manual]]
## Live sheet (Foundry)

*Last synced: 2026-05-31 05:58 UTC*

```statblock
name: FarmPig
size: large
type: humanoid
alignment: unaligned
ac: 17
hp: 76
speed: walk 40 ft.
stats: [20, 10, 18, 2, 12, 5]
cr: 4
traits:
- name: Tusks
  desc: "If the target is prone, this attack deals an extra 2d6 damage.. Melee Weapon Attack: +7 to hit"
- name: Spine Burst (Recharge 6)
  desc: Creatures within 15 ft must make a DC 15 DEX save or take 4d6 piercing damage, half on success.
- name: Relentless
  desc: When reduced to 0 HP, the boar drops to 1 HP instead once per day.
```
