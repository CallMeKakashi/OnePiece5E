---
type: actor
faction: "[[Blackhand]]"
status: draft
publish: true
sources:
  - "Discord/exports/bounty-posters"
  - "Discord/exports/character-art"
  - "Discord/exports/devil-fruit-dex"
foundry_template_json: "Foundry/actors-json/tray.json"
foundry_actor_id: "YtCEWUHoWQrPUddY"
foundry_live_slug: "tray"
---
## Visuals

![[Attachments/tray_caneheart-portrait.png|Portrait]]
![[Attachments/tray-bounty.png|Bounty poster]]

*Discord quarry: [[Discord/exports/character-art]], [[Discord/exports/bounty-posters]]*


## Description

One of the Blackhand pirates, loyal and dependable in the heat of the mission.

## Role

[[Blackhand]] crew member. Part of [[Mira the Unbreakable]]'s inner circle — "the Fleet" — alongside **Thompson**, **Celine**, and **Katherine**.

Devil Fruit: [[Foji Foji no Mi]].

**Latest session:** Brook Town ravine caves — slew **[[Carrion Vulture|Carrion Vultures]]** and **[[Crystalback Scorpion|Crystalback Scorpions]]** with [[Baptiste]] and [[Cade Tigor Cooper]] (transcript pending).

**Prior:** Slew **[[Hydra Goose]]** on West Town farm with [[Matthew -The Jack- Burgess|Jack]] and [[Midori]] (transcript pending).

## Personal Quests

- Standing with Mira and the Fleet through whatever comes

## Backstory

Tray is one of [[Mira the Unbreakable]]'s closest allies. He was part of the group that helped Mira escape **CP0** torture — the tight-knit circle she calls "the Fleet" (Tray, Thompson, Celine, and Katherine). To them, Mira is nothing short of a god: the one who showed them a better life far from the slums of an unnamed island deep within the New World.

Tray was also present during the raid on the Germa facility where [[Baptiste]] was rescued, fighting alongside [[Blackhand Cane]], [[Renzo]], [[Morley]], and others.
## Build template (Foundry)

> **Build template only** — not the live Foundry sheet. Running stats live in the Pi world `blood-and-brine` → `data/actors/`.

Workshop JSON (import/build): `[[Foundry/actors-json/tray.json]]`.
## Live sheet (Foundry)

*Last synced: 2026-05-31 05:10 UTC*

```statblock
name: Tray
size: medium
type: humanoid
alignment: neutral good
ac: 13
hp: 65
speed: walk 30 ft.
stats: [12, 14, 14, 10, 13, 15]
cr: 3
skills:
- Acrobatics: +4
- Insight: +4
- Medicine: +6
- Persuasion: +4
traits:
- name: Forge-Fist Healing
  desc: "Tray channels the Forge-Forge Fruit through his fists. Melee touch: heals a creature for 2d8 + 2 HP. Can also cure minor injuries, scars, or broken bones over time (DM discretion)."
- name: Forge-Fist Strike
  desc: Untrained strike using his fruit. Tray dislikes fighting and lacks proper technique.
- name: Reforge Body
  desc: Tray can alter his appearance by reshaping his body via touch. Functions like a disguise (as Disguise Self but physical). Lasts until he changes it again.
- name: Flinch Reaction
  desc: When targeted by an attack, Tray can use his reaction to impose disadvantage on that attack as he instinctively recoils. After doing so, he has disadvantage on his next attack.
- name: Pacifist Nature
  desc: Tray avoids violence. He will always prioritize healing, fleeing, or protecting others over attacking.
```
