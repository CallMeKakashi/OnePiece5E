---
type: actor
faction: "[[Mugen Industries]]"
status: draft
publish: true
sources:
  - "Discord/exports/character-art"
  - "[[Timeline/1478/1478-12moon [Newspaper] Grand Line Tribune — Morning]]"
---

## Visuals

*Portrait: [[Discord/exports/character-art]] (RyeNera.png — not yet in `Attachments/`).*

## Description

Grotesque **two-human amalgam** — Nikolai's earlier fusion prototype before the three-creature **[[Monster Manual/Aberrant Abomination|Aberrant Abomination]]**. Built by **[[Dr Nikolai Tesla]]** ("The Doctor") at the **Callisto** research facility.

## Role

- Prototype in Nikolai's quest to replicate the **perfect being** he saw at **Virellis** before leaving **Germa 66**.
- Weaker than the Aberrant Abomination (two subjects vs three).
- **Status: dead** (Discord character registry).
- Linked to **[[Hallow]]**, station master / quartermaster at Callisto, and the facility's cogwork / chimera program ([[Episode 6 - Hallow's End]] parallel site **G-45**).

## Backstory

RyeNera was part of the Callisto founding-era experiments. Survivors of the facility collapse blamed pirate sabotage; Mugen and **Commodore Briggs** used the disaster for political cover ([[Timeline/1478/1478-12moon [Newspaper] Grand Line Tribune — Morning]]).

## Related

- [[Dr Nikolai Tesla]]
- [[Monster Manual/Aberrant Abomination|Aberrant Abomination]]
- [[Hallow]] · [[Mugen Industries]]
- [[Timeline/1478/1478-12moon [Newspaper] Grand Line Tribune — Morning|Callisto collapse press]]
