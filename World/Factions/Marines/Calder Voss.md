---
type: actor
faction: "[[Marines]]"
status: draft
publish: true
sources:
  - "[[Episode 5 - Shackles Broken]]"
  - "[[Episode 21 - House of Justice]]"
  - "[[Episode 22 - The Missing Piece]]"
  - "[[Episode 23 - Choice for life]]"
  - "[[Episode 25 - Fire Storm]]"
  - "[[1478-03moon [Newspaper] Blackhand and Sixfold — Spirit Cliff]]"
  - "Discord/exports/character-art"
foundry_template_json: "Foundry/actors-json/vorro.json"
foundry_actor_id: "Ir84x3cJH8bAEbNe"
foundry_live_slug: "calder-voss"
---
## Visuals

![[Attachments/calder_voss-portrait.png|Portrait]]

*Discord quarry: [[Discord/exports/character-art]]*


## Aliases

- **Vorro** — Motley Crew-era name (founding crew period)
- **Calder Voss** — Marine service name (G-45 captain, later commodore)
- Transcript speech-to-text: "Warro" / "Morrow"

## Description

Marine officer who began as a founding member of the [[Motley Crew]] under the name **Vorro**.

## Role

**Marine Captain** at [[Spirit Cliff]] (G-45), later promoted to **Commodore** on judiciary islands.

## Personal Quests

(To be developed from episode threads)

## Backstory

Founding member of the [[Motley Crew]], now serving in the [[Marines]].

## Episode 5

- Commands response to the **[[Worgoman Bank]]** raid; first-class marines under his orders.
- Opposed by colliding [[Sixfold]] and Blackhand-linked crew; clash unresolved at session end.

## Episodes 21–25 (Lex Imperia)

- **Commodore** rank on judiciary islands; G-45 armored Marine under his command ([[Episode 21 - House of Justice]]).
- Captured Ep 22; stripped, branded by [[Baptiste]]; routing papers taken ([[Episode 22 - The Missing Piece]]–[[Episode 23 - Choice for life]]).
- **Escapes** during [[Episode 25 - Fire Storm]] mist attack.

## Related

- [[Motley Crew]]
- [[Spirit Cliff]]
- [[Worgoman Bank]]
- [[1478-03moon [Newspaper] Blackhand and Sixfold — Spirit Cliff]]
## Build template (Foundry)

> **Build template only** — not the live Foundry sheet. Running stats live in the Pi world `blood-and-brine` → `data/actors/`.

Workshop JSON (import/build): `[[Foundry/actors-json/vorro.json]]`.
## Live sheet (Foundry)

*Last synced: 2026-05-31 05:10 UTC*

```statblock
name: Calder Voss
size: medium
type: humanoid
alignment: lawful neutral
ac: 18
hp: 168
speed: walk 40 ft.
stats: [16, 20, 18, 14, 16, 14]
cr: 10
skills:
- Acrobatics: +7
- Athletics: +7
- Insight: +7
- Perception: +7
- Stealth: +7
traits:
- name: Multiattack
  desc: Calder Voss makes three Shigan attacks.
- name: Shigan (Finger Pistol)
  desc: A Rokushiki finger strike that pierces armor.
- name: Rankyaku (Tempest Kick)
  desc: A flying blade of compressed air fired from a kick in a 60 ft line.
- name: Rokuogan
  desc: A devastating Rokushiki shockwave that deals internal damage. Recharge 5-6.
- name: Soru
  desc: Calder moves up to 30 ft without provoking opportunity attacks.
- name: Geppo
  desc: Calder can jump through the air, allowing vertical repositioning or midair movement.
- name: Tekkai
  desc: Calder hardens his body gaining +3 AC until the start of his next turn.
- name: Legendary Actions
  desc: "Calder Voss can take 3 legendary actions, choosing from the options below. Only one legendary action option can be used at a time and only at the end of another creature's turn."
- name: "Legendary Action: Shigan"
  desc: Calder makes one Shigan attack.
- name: "Legendary Action: Soru Step"
  desc: Calder moves up to 30 ft without provoking opportunity attacks.
- name: "Legendary Action: Rankyaku Slash (Costs 2)"
  desc: Calder unleashes a Rankyaku attack.
```
